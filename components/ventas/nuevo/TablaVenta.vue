  ...
  >
    ...
    <template v-slot:item.subtotal="{ item, index }">
      <v-text-field
        ...
        class="shrink prueba"
        v-on:change="updateSubtotal(item,index)"
      >
        mdi-delete
      </v-text-field>
    </template>
    ...
    updateSupplyRow(item, index){
      this.productsToSelect.forEach(x => {
        if (x.productId === item){
          ...
          this.rows.splice(index, 0, Object.assign({}, {productId: x.productId, soldSacks:'', subtotal:''}));
        }
      });
      this.triggerFillProducts();
    },
